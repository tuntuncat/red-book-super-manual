...
后面经过优化，垃圾回收程序会对回收的内存和全部分配的内存进行一个比例计算。当这个比例较低（15%以下）时，下次垃圾回收启动的阈值就会提高一倍（相当于降低了清理的频率）。当比例较高（85%以上）时，会还原到初始的阈值。这样的动态调整极大的优化了垃圾回收的性能消耗。
...
4. 在大多数当代前端代码规范中(如eslint-Airbnb)，var/const/let的使用优先级是？
5. 什么是作用域链，作用域链？
6. JS引擎常用的垃圾回收方法？
7. 为什么我们要尽量避免声明全局变量？
8. 列举几种常用的内存优化方法？
